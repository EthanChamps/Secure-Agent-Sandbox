...
4. Install an independent watchdog/reaper for names prefixed `sas-`, with admission tracking and a hard TTL. Confirm cleanup after worker termination and Docker CLI timeout. Keep concurrency bounded and reserve controller resources outside workload cgroups.
